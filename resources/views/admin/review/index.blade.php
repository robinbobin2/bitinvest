...
<div class="container">
    <div class="row">
        <div class="col-md-8 col-md-offset-2">
            ...
            <h1>Обзоры</h1>
             <div style="float:right"><a href="{{route('review.create')}}">Добавить</a></div></div>
                      ...
                      <tbody>
                        @foreach($news as $n)
                          <tr>
                              <td>{{$n->title}}</td>
                              <td>
                                  <a href="{{route('review.edit', $n->id)}}">Edit</a> | 
                                  <a href="#" onclick="event.preventDefault();
                                      document.getElementById('delete-news-{{$n->id}}').submit();">
                                      Delete
                                  </a>
                                  <form id="delete-news-{{$n->id}}" action="{{route('review.destroy', $n->id)}}" method="POST" style="display: none;">
                                            {{ csrf_field() }}
